---
name: smart-search
description: Search Yuque documents with natural language queries and provide summarized answers with key points and source links. Use when the user wants to find information, ask questions about, or look up content stored in their Yuque knowledge base.
license: Apache-2.0
...
# Smart Search — Yuque Intelligent Search & Q&A

Search across Yuque knowledge bases using natural language, read relevant documents, and synthesize a clear answer with references.

## When to Use

- User asks a question that might be answered by their Yuque docs
- User wants to find specific information in their knowledge base
- User says "search Yuque for...", "find in Yuque...", "what does our docs say about..."
...
Examples:
- "我们的部署流程是什么？" → keywords: `部署流程`
- "How do we handle error logging?" → keywords: `错误日志` or `日志处理`
...
- Repository context (if user specified a knowledge base)
...
User: "我们的 API 限流策略是什么？"

1. Search: `yuque_search(query="API 限流策略", type="doc")`
2. Found: "API 网关限流配置" in 技术文档库
3. Read: `yuque_get_doc(namespace="tech-docs", slug="api-rate-limit")`
4. Answer with key rate limit numbers, rules, and link to the doc

### Example 2: Broad Topic

User: "帮我了解一下我们的微服务架构"

1. Search: `yuque_search(query="微服务架构", type="doc")`
2. Found multiple docs: architecture overview, service list, deployment guide
3. Read top 3 docs
4. Synthesize an overview covering: architecture pattern, key services, communication method
5. List all relevant docs as references for deeper reading
